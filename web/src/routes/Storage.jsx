...
export default function Storage() {
  ...
  if (
    service &&
    service['storage']['/media/opengate/recordings']['total'] != service['storage']['/media/opengate/clips']['total']
  ) {
    storage_usage = (
      <Fragment>
        <Tr>
          <Td>Recordings</Td>
          <Td>{getUnitSize(service['storage']['/media/opengate/recordings']['used'])}</Td>
          <Td>{getUnitSize(service['storage']['/media/opengate/recordings']['total'])}</Td>
        </Tr>
        <Tr>
          <Td>Snapshots</Td>
          <Td>{getUnitSize(service['storage']['/media/opengate/clips']['used'])}</Td>
          <Td>{getUnitSize(service['storage']['/media/opengate/clips']['total'])}</Td>
        </Tr>
      </Fragment>
    );
  } else {
          ...
          <div className="dark:bg-gray-800 shadow-md hover:shadow-lg rounded-lg transition-shadow">
            <div className="flex justify-start">
              ...
              <Button
                ...
                type="text"
                color="gray"
                aria-label="Overview of used and total memory in opengate process."
              >
                <About className="w-5" />
              </Button>
            </div>
