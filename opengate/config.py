...
from opengate.detectors import DetectorConfig, ModelConfig
from opengate.detectors.detector_config import BaseDetectorConfig
from opengate.ffmpeg_presets import (
...
from opengate.util.image import create_mask
from opengate.util.services import get_video_properties
...
class OpenGateBaseModel(BaseModel):
    class Config:
        extra = Extra.forbid
...
class UIConfig(OpenGateBaseModel):
...
class TelemetryConfig(OpenGateBaseModel):
...
class MqttConfig(OpenGateBaseModel):
    ...
    topic_prefix: str = Field(default="opengate", title="MQTT Topic Prefix")
    client_id: str = Field(default="opengate", title="MQTT Client ID")
...
class EventsConfig(OpenGateBaseModel):
...
class RecordConfig(OpenGateBaseModel):
...
class MotionConfig(OpenGateBaseModel):
    threshold: int = Field(
        default=30,
...
class StationaryMaxFramesConfig(OpenGateBaseModel):
...
class StationaryConfig(OpenGateBaseModel):
...
class FilterConfig(OpenGateBaseModel):
    min_area: int = Field(
...
class AudioFilterConfig(OpenGateBaseModel):
    threshold: float = Field(
        default=0.8,
...
class AudioConfig(OpenGateBaseModel):
...
class BirdseyeLayoutConfig(OpenGateBaseModel):
...
class BirdseyeConfig(OpenGateBaseModel):
...
class FfmpegOutputArgsConfig(OpenGateBaseModel):
...
class FfmpegConfig(OpenGateBaseModel):
...
class CameraInput(OpenGateBaseModel):
...
class SnapshotsConfig(OpenGateBaseModel):
...
class ColorConfig(OpenGateBaseModel):
...
class TimestampStyleConfig(OpenGateBaseModel):
...
class RtmpConfig(OpenGateBaseModel):
...
class CameraUiConfig(OpenGateBaseModel):
    order: int = Field(default=0, title="Order of camera in UI.")
    dashboard: bool = Field(
        default=True, title="Show this camera in OpenGate dashboard UI."
    )
...
class CameraConfig(OpenGateBaseModel):
    ...
    def _get_ffmpeg_cmd(self, ffmpeg_input: CameraInput):
        ...
        cmd_str = " ".join(cmd)
        logging.info(f"FFmpeg command: {cmd_str}")
        return cmd


class DatabaseConfig(OpenGateBaseModel):
...
class LoggerConfig(OpenGateBaseModel):
...
def verify_valid_live_stream_name(
    opengate_config: OpenGateConfig, camera_config: CameraConfig
) -> ValueError | None:
    """Verify that a restream exists to use for live view."""
    if (
        camera_config.live.stream_name
        not in opengate_config.go2rtc.dict().get("streams", {}).keys()
    ):
        return ValueError(
...
class OpenGateConfig(OpenGateBaseModel):
    mqtt: MqttConfig = Field(title="MQTT Configuration.")
    database: DatabaseConfig = Field(
        default_factory=DatabaseConfig, title="Database configuration."
    )
    environment_vars: Dict[str, str] = Field(
        default_factory=dict, title="OpenGate environment variables."
    )
