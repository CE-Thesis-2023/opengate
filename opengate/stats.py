...
from opengate.const import CACHE_DIR, CLIPS_DIR, DRIVER_AMD, DRIVER_ENV_VAR, RECORD_DIR
from opengate.object_detection import ObjectDetectProcess
from opengate.types import CameraMetricsTypes, StatsTrackingTypes
from opengate.util.services import (
    get_amd_gpu_stats,
    get_bandwidth_stats,
    get_cpu_stats,
    get_intel_gpu_stats,
...
def get_processing_stats(
    config: OpenGateConfig, stats: dict[str, str], hwaccel_errors: list[str]
) -> None:
...
async def set_bandwidth_stats(
    config: OpenGateConfig, all_stats: dict[str, Any]
) -> None:
...
async def set_gpu_stats(
    config: OpenGateConfig, all_stats: dict[str, Any], hwaccel_errors: list[str]
) -> None:
...
def stats_snapshot(
    ...
    stats["service"] = {
        ...
        "version": VERSION,
        "latest_version": stats_tracking["latest_opengate_version"],
        "storage": {},
...
class StatsEmitter(threading.Thread):
    def __init__(
        ...
        self.name = "opengate_stats_emitter"
        self.config = config
