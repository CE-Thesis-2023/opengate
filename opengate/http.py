...
from opengate.storage import StorageMaintainer
from opengate.util.builtin import (
    clean_camera_user_pass,
    get_tz_modifiers,
    update_yaml_from_url,
)
from opengate.util.services import ffprobe_stream, restart_opengate, vainfo_hwaccel
...
bp = Blueprint("opengate", __name__)
...
def create_app(
    ...
    app.opengate_config = opengate_config
...
@bp.route("/")
def is_healthy():
    return "OpenGate is running. Alive and healthy!"
...
def grid_snapshot(camera_name):
    ...
    if camera_name in current_app.opengate_config.cameras:
        detect = current_app.opengate_config.cameras[camera_name].detect
        frame = current_app.detected_frames_processor.get_current_frame(camera_name, {})
        retry_interval = float(
            current_app.opengate_config.cameras.get(camera_name).ffmpeg.retry_interval
            or 10
        )
...
def create_event(camera_name, label):
    if not camera_name or not current_app.opengate_config.cameras.get(camera_name):
        return make_response(
            jsonify(
...
@bp.route("/config")
def config():
    config = current_app.opengate_config.dict()

    # remove the mqtt password
    config["mqtt"].pop("password", None)

    for camera_name, camera in current_app.opengate_config.cameras.items():
    ...
    for detector, detector_config in config["detectors"].items():
        detector_config["model"]["labelmap"] = (
            current_app.opengate_config.model.merged_labelmap
        )
...
def config_save():
    ...
    try:
        OpenGateConfig.parse_raw(new_config)
    except Exception:
        return make_response(
            jsonify(
                {
    ...
    try:
        ...
        return make_response(
            jsonify(
                {
                    "success": False,
                    "message": "Could not write config file, be sure that OpenGate has write permission on the config file.",
                }
            ),
            400,
        )
    ...
    if save_option == "restart":
        try:
            ...
            logging.error(f"Error restarting OpenGate: {e}")
            return make_response(
                jsonify(
                    {
                        "success": True,
                        "message": "Config successfully saved, unable to restart OpenGate",
                    }
                ),
                200,
            )
...
def config_set():
    ...
    try:
        ...
        # Validate the config schema
        try:
            OpenGateConfig.parse_raw(new_raw_config)
        except Exception:
            with open(config_file, "w") as f:
...
def config_schema():
    return current_app.response_class(
        current_app.opengate_config.schema_json(), mimetype="application/json"
    )
...
def latest_frame(camera_name):
    ...
    if camera_name in current_app.opengate_config.cameras:
        ...
        if frame is None or datetime.now().timestamp() > (
            ...
            if current_app.camera_error_image is None:
                error_image = glob.glob(
                    "/opt/opengate/opengate/images/camera-error.jpg"
                )
        ...
        response.headers["Cache-Control"] = "no-store"
        return response
    elif camera_name == "birdseye" and current_app.opengate_config.birdseye.restream:
        frame = cv2.cvtColor(
...
def get_snapshot_from_recording(camera_name: str, frame_time: str):
    if camera_name not in current_app.opengate_config.cameras:
        return make_response(
            jsonify({"success": False, "message": "Camera not found"}),
            404,
        )
...
def ffprobe():
    ...
    if path_param.startswith("camera"):
        ...
        if camera not in current_app.opengate_config.cameras.keys():
            return make_response(
                jsonify(
        ...
        if not current_app.opengate_config.cameras[camera].enabled:
            return make_response(
        ...
        paths = map(
            ...
            current_app.opengate_config.cameras[camera].ffmpeg.inputs,
        )
...
def logs(service: str):
    log_locations = {
        "opengate": "/dev/shm/logs/opengate/current",
    ...
    try:
        ...
        return make_response(
            jsonify(
                {
                    "success": False,
                    "message": "Unable to restart OpenGate.",
                }
            ),
            500,
        )
