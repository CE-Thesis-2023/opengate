...
from opengate.util.builtin import clean_camera_user_pass, escape_special_characters

logger = logging.getLogger(__name__)


def restart_opengate():
    proc = psutil.Process(1)
    # if this is running via s6, sigterm pid 1
    if proc.name() == "s6-svscan":
        proc.terminate()
    # otherwise, just try and exit opengate
    else:
...
def get_bandwidth_stats(config) -> dict[str, dict]:
    ...
    if p.returncode != 0:
        ...
        for line in lines:
            ...
            try:
                if re.search(
                    r"(^ffmpeg|\/go2rtc|opengate\.detector\.[a-z]+)/([0-9]+)/", stats[0]
                ):
